        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border bg-muted/30">
              {['Name', 'Email', 'Phone', 'Company', 'Country', 'Created', ''].map(h => (
          ...
          <tbody>
            {isLoading ? Array.from({ length: 8 }).map((_, i) => (
              ...
              <tr key={c.id} className="border-b border-border last:border-0 hover:bg-muted/20 transition-colors">
                <td className="px-4 py-3 font-medium text-foreground text-sm">{c.first_name} {c.last_name}</td>
                ...
                <td className="px-4 py-3 text-xs text-muted-foreground">{c.company_name || '—'}</td>
        ...
        <DialogContent className="max-w-lg">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="col-span-2">
              ...
              <Input className="h-8 text-sm mt-1" value={form.address ?? ''} onChange={e => setForm((f: any) => ({ ...f, address: e.target.value }))} />
            </div>
          </div>
          <DialogFooter>
